...
func apply_upgrade(upgrade: AbilityUpgrade):
	...
	if upgrade.max_quantity > 0:
		var current_max_quantity = current_upgrades[upgrade.id]["quantity"]
		if current_max_quantity == upgrade.max_quantity:
			# Out the ability from the pool choose option possible
			upgrade_pool = upgrade_pool.filter(func (pool_upgrade): return pool_upgrade.id != upgrade.id)
	
	Globals.ability_upgrade_added.emit(upgrade, current_upgrades)
